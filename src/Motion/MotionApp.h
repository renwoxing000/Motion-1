...
#define APPLICATION_WINDOW_TITLE "Motion"
#undef USE_RTSHADER_SYSTEM

/*=============================================================================
| The Motion Application. Features a menu accessible from all activities,
| dynamic configuration, resource reloading, node labeling, and more.
=============================================================================*/
class MotionApp : public SampleContext, public SdkTrayListener
{
public:
	MotionApp(int startSampleIndex = -1) : SampleContext()
	{
		mNoGrabInput = false;
		mTrayMgr = 0;
		mLastViewCategory = 0;
...
protected:
	...
	{
		Ogre::RenderWindow* res = mRoot->initialise(true, APPLICATION_WINDOW_TITLE);
		return res;
    }
...
protected:
	...
	OgreBites::SampleSet mLoadedSamples;           // loaded samples
	OgreBites::SelectMenu* mCategoryMenu;          // sample category select menu
	OgreBites::SelectMenu* mSampleMenu;            // sample select menu
	OgreBites::Slider* mSampleSlider;              // sample slider bar
	OgreBites::Label* mTitleLabel;                 // sample title label
	OgreBites::TextBox* mDescBox;                  // sample description box
	OgreBites::SelectMenu* mRendererMenu;          // render system selection menu
